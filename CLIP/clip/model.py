...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        scale = width ** -0.5
        self.class_embedding = nn.Parameter(scale * torch.randn(width)) # embedding 大小 width
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = self.ln_post(x[:, 0, :]) # 取出class token进行映射
...
class CLIP(nn.Module):
    ...
    def encode_text(self, text): # text:[3,77]
        x = self.token_embedding(text).type(self.dtype)  # [batch_size, n_ctx, d_model]:[3,77,512]

        x = x + self.positional_embedding.type(self.dtype) # self.positional_embedding.type(self.dtype):[77,512]
        x = x.permute(1, 0, 2)  # NLD -> LND
        ...
        x = x[torch.arange(x.shape[0]), text.argmax(dim=-1)] @ self.text_projection # (3,512)*(512,512)
        # text.argmax(dim=-1)：表示三个batch当中最大长度
        return x
    ...
    def forward(self, image, text):
        ...
        logits_per_image = logit_scale * image_features @ text_features.t() # [1,3] 行表示图像，列表示text
